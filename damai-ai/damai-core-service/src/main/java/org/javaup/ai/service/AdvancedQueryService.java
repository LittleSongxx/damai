...
public class AdvancedQueryService {
    ...
    private static final Map<String, String> ENTITY_ALIASES = Map.ofEntries(
            ...
            Map.entry("儿童", "儿童购票政策 亲子票 儿童免票"),
            Map.entry("相机", "摄影摄像设备 专业器材 拍摄限制"),
            Map.entry("支付超时", "订单支付时限 未支付取消 保留订单"),
            Map.entry("重复支付", "重复扣款 多付退款 重复订单处理"),
            Map.entry("填错", "修改信息 信息变更 更正观演人"),
            Map.entry("演出取消", "演出取消 自动退款 取消公告"),
            Map.entry("快递", "配送进度 物流查询 快递签收"),
            Map.entry("选座", "在线选座 座位图 座位锁定"),
            Map.entry("退票手续费", "退票费率 阶梯退票 手续费标准")
    );
    ...
    public String generateHypotheticalDocument(String query) {
        ...
        try {
            String prompt = """
                    ...
                    【重要】必须至少输出300字，这是严格的硬性要求。生成的内容必须包含：
                    - 规则适用条件
                    - 具体操作步骤（至少3步）
                    - 注意事项和限制条件
                    - 相关补充信息
            ...
            if (StringUtils.hasText(result)) {
                ...
                log.warn("HyDE 文档过短({}字)，使用原始查询替代", trimmed.length());
            }
            return query;
        } catch (Exception e) {
    ...
    public String extractSearchKeywords(String query) {
        ...
        try {
            String prompt = """
                    ...
                    规则：
                    1. 优先提取领域术语（退票、实名制、电子票、选座、支付、配送、儿童购票等）
                    2. 补充同义表达和规范写法（退货→退票退款、证件→身份证实名制、不能退→不支持退票）
                    3. 同时包含问题中的核心名词和对应的操作动词
                    4. 每个关键词不超过8个字
                    5. 用空格分隔，不要编号

...
ai_db_initialized(){ local a; a="$(mysql_exec "SELECT COUNT(*) FROM information_schema.TABLES WHERE TABLE_SCHEMA='damai_ai' AND TABLE_NAME IN ('d_ai_session','d_ai_workflow_run','d_ai_workflow_step','d_ai_approval','d_ai_tool_audit','d_ai_retrieval_trace','d_ai_trace');" 2>/dev/null | tr -d '\r[:space:]' || true)"; [[ "$a" == "7" ]]; }
service_jar(){ find "$1/target" -maxdepth 1 -type f -name '*.jar' ! -name 'original-*' ! -name '*-sources.jar' -printf '%T@ %p\n' | sort -nr | head -n 1 | cut -d' ' -f2-; }
start_bg(){ local name="$1" port="$2" mode="$3" target="$4" timeout="$5" cwd="$6" pidfile logfile; shift 6; pidfile="$PID_DIR/$name.pid"; logfile="$LOG_DIR/$name.log"; if pid_alive "$pidfile"; then log "$name already running (PID $(cat "$pidfile"))"; elif port_listening "$port"; then log "Reusing existing $name on $port"; rm -f "$pidfile"; else log "Starting $name"; ( cd "$cwd"; setsid nohup "$@" </dev/null >>"$logfile" 2>&1 & echo $! > "$pidfile" ); fi; wait_ready "$name" "$mode" "$target" "$timeout"; }
...
print_addresses(){
  ...
  echo "  qdrant:               http://127.0.0.1:${DAMAI_QDRANT_PORT:-16333}"
